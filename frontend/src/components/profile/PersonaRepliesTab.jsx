...
export default function PersonaRepliesTab({ handle }) {
  ...
  const load = async (pageNum = page) => {
    try {
      const res = await api.get(`/personas/${handle}/replies`, {
      ...
      console.error('Load persona replies error:', e);
      setHasMore(false);
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      {replies.map((reply) => {
        ...
        const contextText = hasParent ? reply.parentComment?.content : reply.thread?.content || '—';
